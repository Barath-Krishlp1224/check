...
const TasksPage: React.FC = () => {
  ...
  // --- FILTER: Show only ACCOUNTS team members ---
  ...
  // --- FILTER: Show only ACCOUNTS department tasks ---
  ...
  const handleExcelDownload = useCallback(() => {
    ...
    XLSX.writeFile(workbook, `Accounts_Report_${downloadFilterValue || 'Data'}.xlsx`);
  ...
  if (loading || !minLoadTimePassed) return (
    <div className="flex justify-center items-center min-h-screen bg-white">
      <div className="text-center">
        ...
        <p className="text-slate-700 font-medium">Synchronizing Accounts Resource Data...</p>
      </div>
    </div>
  );
              ...
              <div className="w-full lg:w-72">
                <div className="sticky top-32 bg-white rounded-3xl shadow-xl border border-slate-100 p-2">
                  <h2 className="text-[10px] font-black text-slate-400 uppercase p-4 tracking-widest flex items-center gap-2">
                    <User className="w-4 h-4 text-indigo-500"/> Capacity (Accounts)
                  </h2>
