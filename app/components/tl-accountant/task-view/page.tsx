...
const TasksPage: React.FC = () => {
  ...
  const filteredTasks = useMemo(() => {
    ...
    // --- START: DEPARTMENT FILTER LOGIC (UPDATED) ---
    const allowedDepartments = ["Accounts", "TL Accountant"];

    // Filter tasks to include only allowed departments
    const departmentTasks = tasks.filter(task => 
        task.department && allowedDepartments.includes(task.department)
    );
    
    let currentTasks = departmentTasks;
    ...
    if (filter === "all" && !primaryFilterValue && !fromDateString && !toDateString) {
      return currentTasks; // Return all Accounts/TL Accountant tasks if no other filters are applied
    }
  ...
  }, [tasks, downloadFilterType, downloadFilterValue]); // Depend on the original tasks list
  ...
  const handleExcelDownload = () => {
    ...
    if (filter === "all" || !value) {
      tasksForExport = departmentTasks; // Export all Accounts/TL Accountant tasks
    } else {
      // Apply existing filters to the filtered tasks only
      tasksForExport = departmentTasks.filter((task) => {
        switch (filter) {
          case "project":
    ...
    if (tasksForExport.length === 0) {
      alert(`No Accounts/TL Accountant tasks found for the current filter: ${filter} = ${value || "N/A"}`);
      return;
    }
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Accounts/TL Accountant Tasks Report");

    const safeValue = value.replace(/[^a-z0-9]/gi, "_");
    const fileName = filter === "all" ? "Accounts_TL_All_Tasks_Report.xlsx" : `Accounts_TL_${filter}_${safeValue}_Tasks_Report.xlsx`;
          ...
          {filteredTasks.length === 0 ? (
            <div className="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden">
              <div className="text-center py-16">
                ...
                <h3 className="text-xl font-semibold text-slate-700 mb-2">No Accounts/TL Accountant tasks found</h3>
                <p className="text-slate-500">The current filter returned no matching tasks for Accounts or TL Accountant departments.</p>
              </div>
            </div>
          ) : (
            <>
