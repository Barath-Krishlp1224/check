...
const TaskTableHeader: React.FC<TaskTableHeaderProps> = ({
  ...
  const handleDateRangeChange = (field: "from" | "to", dateValue: string) => {
    ...
    const primaryValue =
      downloadFilterType === "assignee"
        ? selectedAssignee
        : downloadFilterValue.split("|")[0] || downloadFilterType;
  ...
  const handleDurationSelect = (months: number) => {
    ...
    const newFromDate = formatDate(pastDate);
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border border-slate-100">
        <div className="flex items-end justify-between">
          ...
          <div className="flex items-center gap-4 bg-white rounded-xl shadow-lg border border-slate-200 p-4">
            ...
            {downloadFilterType === "assignee" && (
              ...
              >
                ...
                {employees.map((employee) => (
                  <option key={employee._id} value={employee.name.toLowerCase()}>
                    {employee.name}
                  </option>
                ))}
              </select>
            ...
            {showOptionalDateFilters &&
              (downloadFilterType !== "assignee" || (selectedAssignee && selectedAssignee !== "all")) && (
