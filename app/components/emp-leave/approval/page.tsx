...
const ApprovalPage: React.FC = () => {
  ...
  // Sort non-pending requests by creation date descending to show most recent first
  const sortedNonPendingRequests = requests
    .filter((r) => r.status !== "pending")
    .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());

  // Show only the first 5 for history list, even if there are more
  const recentFiveNonPending = sortedNonPendingRequests.slice(0, 5);
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center py-12 px-4">
      <div className="max-w-7xl w-full">
        ...
        <p className="text-gray-600 text-sm mb-6">
          View all leave requests submitted by employees. Action buttons are
          only shown for Pending requests.
        </p>

        <div
          className="bg-white rounded-xl shadow-md p-5 mb-8 border border-gray-200"
        >
        ...
        <div className="space-y-8">
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6">
            <h2 className="text-xl font-bold text-gray-900 mb-4">
              Leave History ({sortedNonPendingRequests.length})
            </h2>

            {loading && sortedNonPendingRequests.length === 0 ? (
              <p className="text-gray-600 text-sm">Loading history...</p>
            ) : sortedNonPendingRequests.length === 0 ? (
              <p className="text-gray-600 text-sm">
                No approved, rejected, or auto-approved leave requests found{" "}
                {isFilterActive ? "for current filters" : ""}.
              </p>
            ) : (
              // Applied max-height and overflow for scrolling
              <div className="overflow-x-auto max-h-[300px] overflow-y-auto">
                <table className="min-w-full text-sm text-gray-900">
                  ...
                  <tbody>
                    {/* Only mapping over the sliced array */}
                    {recentFiveNonPending.map((req) => (
                      <tr
                        key={req._id}
                    ...
                    {/* Add a placeholder row if there are more than 5 items */}
                    {sortedNonPendingRequests.length > 5 && (
                      <tr className="border-b last:border-0 bg-gray-100">
                        <td colSpan={7} className="py-2 text-center text-gray-600 italic">
                          Scroll down to see the remaining {sortedNonPendingRequests.length - 5} history entries.
                        </td>
                      </tr>
                    )}
                  </tbody>
